...
export default function IndustryExp() {
  ...
  const onSubmit = (e) => {
    ...
    axios
      .post(`/applications/${appId}/indexp`, data)
      .then((res) => {
    ...
    <AppLayout>
      <div className="flex justify-around mb-2 p-3 border">
        <Link to={`/dashboard/application/acadexp/${appId}`}>
          <button className="border-b-4 p-2 text-white font-bold bg-indigo-600">Academic Experience</button>
        </Link>
        <Link to={`/dashboard/application/indexp/${appId}`}>
          <button className="border-b-4 border-black p-2 text-white font-bold bg-indigo-600">Industry Experience</button>
        </Link>
      </div>
      <form id="indxpform" onSubmit={onSubmit}>
        <div className="editor w-screen mb-10 w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
        <h1 className="text-2xl text-indigo-600 mb-4">Industry Experience</h1>
